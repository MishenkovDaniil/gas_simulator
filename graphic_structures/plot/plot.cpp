...
Plot::Plot (Point &start, int x_len, int y_len, Color color) 
          : size_ (x_len),
            start_ (start), 
            color_ (color),
            ordinate_len_ (y_len),
...
void Plot::draw (sf::RenderTarget &target, double param)
{
    ...
    if (!is_filled)
    {
        array[cur].position = sf::Vector2f (start_.x_  + cur, start_.y_ - param);
        array[cur].color = sf::Color::Red;
        points_[cur] = Point (start_.x_ + cur, start_.y_ + param);
        cur++;
    }
    else
    {
        ...
        points_[size_ - 1] = Point (start_.x_ + size_ - 1, start_.y_ - param);
        array[size_ - 1].position.y = start_.y_ - param;
    }
